...
//    }
    ...
    public void saveMarket(net.chen.legacyLand.market.Market market) {
        String sql = "INSERT INTO markets (id, nation_name, world, chunk_x, chunk_z, approved_by, created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?) " +
                "ON DUPLICATE KEY UPDATE nation_name=VALUES(nation_name), world=VALUES(world), " +
                "chunk_x=VALUES(chunk_x), chunk_z=VALUES(chunk_z)";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public void saveMarketChest(net.chen.legacyLand.market.MarketChest chest) {
        String sql = "INSERT INTO market_chests (id, market_id, world, x, y, z, owner_uuid, price_per_item, price_set, created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                "ON DUPLICATE KEY UPDATE price_per_item=VALUES(price_per_item), price_set=VALUES(price_set)";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
